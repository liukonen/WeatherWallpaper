...
namespace ExternalService.template
{
    ...
    class SunRiseSetClass : IsharedSunRiseSetInterface
    {
        ...
        {
             /*Sample code
            if (SharedObjects.Cache.Exists("key")) { return (SunRiseSetResponse)SharedObjects.Cache.Value("key"); }
            string response = SharedObjects.CompressedCallSite("url");
            SunRiseSetResponse responseObject = Transform(response);
